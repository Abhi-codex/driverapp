...
export default function DriverProfile() {
  ...
  const fetchDriverData = async () => {
    try {
      ...
      // Fetch driver profile with Firebase ID token fallback
      const firebaseToken = await AsyncStorage.getItem('firebase_id_token');
      console.log('[PROFILE] Using Firebase ID token for profile fetch');
      ...
      // If 401 with Firebase token, try access token
      if (profileResponse.status === 401) {
        console.log('[PROFILE] Firebase token failed, trying access token...');
        ...
        // If still 401, try to refresh token
        if (profileResponse.status === 401) {
      ...
      if (statsResponse.status === 401) {
        console.log('[PROFILE] Stats: Firebase token failed, trying access token...');
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      // Use Firebase ID token for profile update (since it works for profile updates)
